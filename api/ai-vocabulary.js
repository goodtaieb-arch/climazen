...
async function authorizeOrg(req, orgId) {
  ...
  if (!user?.id) return { ok: false, status: 401, error: 'Session requise.' }
  const profile = await userOrgProfile(user.id)
  if (!profile?.organization_id) {
    return { ok: false, status: 403, error: 'Profil société introuvable.' }
  }
  const requested = String(orgId || profile.organization_id).trim()
  if (requested !== profile.organization_id) {
    return { ok: false, status: 403, error: 'Accès refusé à cette société.' }
  }
  return { ok: true, user, profile, orgId: requested }
}
